    ...
    function getExifDataMarkup(img) {
      ...
      for (var current in exif) {
        ...
        if (typeof exif_data !== "undefined") {
          // Format DateTimeOriginal to show only month/year
          if (current_data["tag"] === "DateTimeOriginal") {
            // Parse date format "2022:04:14 19:24:38" to "04/2022"
            var dateParts = exif_data.split(" ")[0].split(":");
            if (dateParts.length >= 3) {
              exif_data = dateParts[1] + "/" + dateParts[0];
            }
          }
          template +=
            '<i class="fa fa-' +
